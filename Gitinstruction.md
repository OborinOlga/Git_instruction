...
Принято в отслеживание добавлять только текстовые файла, а большые файлы, например, картинки нет. А в удаленных репозиториях вообще принято использовать ссылки (относительные) вместо файлов-картинок.
Значит в папку изначально копируем файл-картинку, потом в папке создаем файд ` .gitignore `.

Открыть созданный файл и записать его имя файла-картинки с расширением.

Все файлы записанные и `.gitignore` игнорируются.
