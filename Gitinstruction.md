...
Бывает, что вы поторопились и внесли не все изменения которые должны попасть в коммит, и вы это вспомнили после того как выполнили команду `git add`, но до выполнения компанды `git commit`, тогды мы можем воспользоваться командот отмены добавления к сохранению:
```
git restore <file>
```
