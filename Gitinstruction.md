...
git branch -D <branch name>
```
## 21. Копирование удаленного репозитория
...
```
После выполнения команды внутри ранее созданной папки создается папка с именем выбранного на Github удаленного репозитория. Перемещаемся во вновь созданную папку  по команде:
```
cd <name new folder>
```
## 22. Создание репозитория на Github (1-й способ)
Нужно отправить впервые локальный репозиторий на Github: для этого нужно сохранить и закоммитить все последние изменения в копируемом репозитории, далее последовательно выполняем три команды:
```
